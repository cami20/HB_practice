...
SELECT orders.id, orders.status, orders.order_total FROM orders LEFT JOIN customers ON orders.customer_id = customers.id WHERE customers.email = 'pclark74@gmail.com';
...
SELECT melons.common_name, melons.melon_type, order_items.quantity, order_items.unit_price, order_items.total_price FROM melons LEFT JOIN order_items ON melons.id = order_items.melon_id WHERE order_items.order_id = 2725;
